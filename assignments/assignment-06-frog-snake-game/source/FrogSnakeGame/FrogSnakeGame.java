 ...
 *
 *  The game also has two snakes that constantly move in a random direction
...
public class FrogSnakeGame{
    ...
    // Fields to store the position of the centre of the bug
    private double xBug;
    private double yBug;
    ...
    private void restartGame(){
        ...
        while (this.frog1!=null && this.frog2!=null) {
            ...
            this.snake1.move();
            this.snake2.move();
            if (this.frog1.isTouching(this.xBug, this.yBug)) {
                this.frog1.grow();
                this.newBug();
            }
            else if (this.frog2.isTouching(this.xBug, this.yBug)) {
                this.frog2.grow();
                this.newBug();
            }
            if (this.frog1.isTouching(this.snake1.getX(), this.snake1.getY()) ||
            this.frog1.isTouching(this.snake2.getX(), this.snake2.getY()) ) {
                this.frog1.shrink();
            }
            if (this.frog2.isTouching(this.snake1.getX(), this.snake1.getY()) ||
            this.frog2.isTouching(this.snake2.getX(), this.snake2.getY()) ) {
                this.frog2.shrink();
            }
            this.redraw();
            UI.sleep(DELAY);
    ...
    /**
     * Defines new position for the bug
     */
    private void newBug(){
    ...
    /**
     * Create a new FrogSnakeGame object and sets up the interface
     * The game starts when the player press on the "space" key
     */
